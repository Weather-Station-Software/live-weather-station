        ...
        <table cellspacing="0" class="lws-settings">
                        ...
                        <span class="select-option">
                            <select class="option-at-100" name="plan">
                                <?php foreach ($owm_plan as $plan) { ?>
                                    <option value="<?php echo $plan[0] ?>"<?php if (get_option('live_weather_station_owm_plan')==$plan[0]):?> selected="selected"<?php endif;?>><?php echo $plan[1] ?></option>;
                                <?php } ?>
                            </select>
                        </span>
                    </td>
            ...
            <?php } else {?>
                <tr>
                    ...
                    <td width="2%"/>
                    <td align="left">
                        <span><?php esc_html_e('Up and running' ,'live-weather-station');?> (<a href="<?php echo esc_url(get_admin_page_url('lws-events', null, null, 'OpenWeatherMap')); ?>"<?php echo $target; ?>><?php echo strtolower(__('See events log', 'live-weather-station')); ?></a>)</span>
                    </td>
                </tr>
                <tr>
                    ...
                    <td width="2%"/>
                    <td align="left">
                        <span><?php echo $plan_name ?> (<?php echo InlineHelp::get(-3, '%s', __('get details', 'live-weather-station'));?>)</span>
                    </td>
                </tr>
            <?php } ?>
            </tbody>
    ...
    <?php if (get_option('live_weather_station_owm_apikey') == '') { ?>
        <div id="major-publishing-actions">
            <div id="publishing-action">
                <input type="submit" name="connect" id="connect" class="button button-primary" value="<?php esc_attr_e('Connect', 'live-weather-station');?>">
            </div>
            <div class="clear"></div>
        </div>
    <?php } else {?>
        <div id="major-publishing-actions">
            <div id="publishing-action">
                <input type="submit" name="disconnect" id="disconnect" class="button button-primary" onclick="return confirm('<?php echo $warning; ?>');" value="<?php esc_attr_e('Disconnect', 'live-weather-station');?>">
            </div>
            <div class="clear"></div>
        </div>
    <?php } ?>
