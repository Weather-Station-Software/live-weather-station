...
$map = ' ('.sprintf('<a href="https://maps.google.com?q=%s,%s" target="_blank">'.__('verify on a map', 'live-weather-station').'</a>',$station['loc_latitude'],$station['loc_longitude']).')';
...
<div class="wrap">
    <?php if ($station['station_id'] == 0) { ?>
        <h2><?php esc_html_e( 'Add an OpenWeatherMap station', 'live-weather-station' );?></h2>
    <?php } ?>
    <?php if ($station['station_id'] != 0) { ?>
        <h2><?php esc_html_e( 'Edit an OpenWeatherMap station', 'live-weather-station' );?></h2>
    <?php } ?>
    <?php if ($error == 3) { ?>
        <p style="color:red;"><?php esc_html_e( 'Some required fields are missing!', 'live-weather-station' );?></p>
    <?php } ?>
    <form method="post" name="add-owm" id="add-owm" action="<?php echo esc_url( $this->get_page_url('manage_owm') ); ?>">
        ...
        <input type="hidden" name="action" value="do-add-edit-owm">
        <?php if ($station['station_id'] == 0) { ?>
            <p class="submit"><input type="submit" name="add-owm" id="add-owm" class="button button-primary" value="<?php esc_html_e( 'Add This Station', 'live-weather-station' );?>"  /></p>
        <?php } ?>
        <?php if ($station['station_id'] != 0) { ?>
            <p class="submit"><input type="submit" name="add-owm" id="add-owm" class="button button-primary" value="<?php esc_html_e( 'Save Changes', 'live-weather-station' );?>"  /></p>
        <?php } ?>
    </form>
</div>