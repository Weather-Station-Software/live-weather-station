        ...
        <table cellspacing="0" class="lws-settings">
                        ...
                        <span class="select-option">
                            <select class="option-at-100" name="plan">
                                <?php foreach ($maptiler_plan as $plan) { ?>
                                    <option value="<?php echo $plan[0] ?>"<?php if (get_option('live_weather_station_maptiler_plan')==$plan[0]):?> selected="selected"<?php endif;?>><?php echo $plan[1] ?></option>;
                                <?php } ?>
                            </select>
                        </span>
                    </td>
            ...
            <?php } else {?>
                <tr>
                    ...
                    <td width="2%"/>
                    <td align="left">
                        <span><?php esc_html_e('Up and running' ,'live-weather-station');?> (<a href="<?php echo esc_url(lws_get_admin_page_url('lws-events', null, null, 'Maptiler')); ?>"<?php echo $target; ?>><?php echo lws_lcfirst(__('See events log', 'live-weather-station')); ?></a>)</span>
                    </td>
                </tr>
                <tr>
    ...
    <?php } else {?>
        <div id="major-publishing-actions">
            <div id="publishing-action">
                ...
                <input type="submit" name="disconnect" id="maptiler-disconnect" class="button button-primary" onclick="lws_maptiler_confirmation = confirm('<?php echo $warning; ?>'); return lws_maptiler_confirmation;" value="<?php esc_attr_e('Disconnect', 'live-weather-station');?>">
            </div>
            <div class="clear"></div>
        </div>
    <?php } ?>
