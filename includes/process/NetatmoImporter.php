...
class NetatmoImporter extends Process {
        ...
        foreach ($rows as $row) {
            if ($row['module_type'] === 'NAMain' ||
                ...
                if ($this->params['init']['end_date'] < $old) {
                    ...
                    $days_todo = 1 + (int)floor(($this->params['init']['end_date'] - max($old, $this->params['init']['start_date'])) / 86400);
                }
                $module = array('module_id' => $row['module_id'], 'module_name' => $row['module_name'], 'module_type' => $row['module_type'], 'start_date' => $old);
    ...
    /**
     * Computes the data sets for a module.
     *
     * @param array $data An array containing reference values.
     * @return array An array of timestamped values.
     * @since 3.7.0
     */
    private function compute_datasets($data){
        $result = array();
    ...
    private function run_ext(){
        ...
        $nacomputed['meta'] = array('module_id' => self::get_computed_virtual_id($this->params['init']['station_id']), 'module_name' => __('[Computed Values]', 'live-weather-station'), 'module_type' => 'NAComputed', 'start_date' => null);
        if (count($this->params['todo_ext']) > 0) {
            foreach ($this->params['todo_ext'] as $module) {
                ...
                if ($query_end > $this->params['process']['end_date']) {
                    $query_end = $this->params['process']['end_date'] + 86400 -1;
                }
                if ($module['start_date'] > $query_end) {
                    $this->params['summary'][$module['module_id']]['days_done'] = $this->params['summary'][$module['module_id']]['days_todo'];
                }
                else {
                    $this->get_measures($this->params['init']['station_id'], $module['module_id'], '30min', $this->available_types[$module['module_type']], $query_start, $query_end, 1024, false);
                    switch ($module['module_type']) {
                        case 'NAMain':
            ...
            //error_log(print_r($nacomputed, true));
    ...
    protected function run_core(){
        ...
        elseif (count($this->params['todo_int']) > 0) {
            $this->run_int();
        }
        $this->summarize();
    }
