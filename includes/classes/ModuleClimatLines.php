...
class Lines extends \WeatherStation\Engine\Module\Maintainer {
    ...
    protected function prepare() {
        $js_array_climatline = $this->get_all_stations_array(false, false, true, true, true, false, false, false, array($this->station_guid), false, false, false, false, false, false, true);
    ...
    protected function get_datasource() {
        ...
        for ($i=1; $i<=$this->series_number; $i++) {
            $group = $this->get_neutral_option_select('climat-lines-datas-period-type-'. $i . '-' . $this->station_guid, __('Period', 'live-weather-station'));
            ...
            $a_group[] = array('content' => $group, 'name' => sprintf(lws__('Period %s', 'live-weather-station'), $i));
        }
    ...
    protected function get_parameters() {
        ...
        $content .= $this->get_key_value_option_select('climat-lines-datas-label-'. $this->station_guid, __('Label', 'live-weather-station'), $this->get_multi_2_label_js_array(), true, 'simple');
        ...
        $content .= $this->get_key_value_option_select('climat-lines-datas-timescale-'. $this->station_guid, __('Time scale', 'live-weather-station'), $this->get_x_scale_js_array(true), true, 'auto');
        ...
        $content .= $this->get_key_value_option_select('climat-lines-datas-data-'. $this->station_guid, __('Data', 'live-weather-station'), $this->get_graph_data_js_array(), true, 'inline');
    ...
    protected function get_script() {
        ...
        $content .= '$("#climat-lines-datas-dimension-' . $this->station_guid . '").change();';
