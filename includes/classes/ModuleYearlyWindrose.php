...
/**
 * Class to generate parameter yearly windrose form.
 *
...
class Windrose extends \WeatherStation\Engine\Module\Maintainer {
    ...
    public function __construct($station_information) {
        ...
        $this->module_hint = __('Display historical data as a radial bar chart. Particularly suitable to render wind data as windrose, this graph allows to view the segmented distribution of a measurement according to an angle.', 'live-weather-station');
    ...
    protected function get_datasource() {
        ...
        for ($i=1; $i<=$this->series_number; $i++) {
            ...
            $group .= $this->get_key_value_option_select('yearly-windrose-datas-line-mode-' . $i . '-' . $this->station_guid, __('Allotment', 'live-weather-station'), $this->get_allotment_js_array(), true, '8s', $i != 1);
    ...
    protected function get_parameters() {
        ...
        $content .= $this->get_key_value_option_select('yearly-windrose-datas-color-'. $this->station_guid, __('Color scheme', 'live-weather-station'), $this->get_colorbrewer_js_array(true));
        ...
        $content .= $this->get_key_value_option_select('yearly-windrose-datas-guideline-'. $this->station_guid, __('Control', 'live-weather-station'), $this->get_sareas_group_js_array(), true, 'stacked', true, false);
        ...
        $content .= $this->get_key_value_option_select('yearly-windrose-datas-timescale-'. $this->station_guid, __('Time scale', 'live-weather-station'), $this->get_x_scale_js_array(false), true, 'auto');
        $content .= $this->get_key_value_option_select('yearly-windrose-datas-valuescale-'. $this->station_guid, __('Value scale', 'live-weather-station'), $this->get_y_scale_js_array(true), true, 'auto', true, false);
        $content .= $this->get_key_value_option_select('yearly-windrose-datas-interpolation-'. $this->station_guid, __('Interpolation', 'live-weather-station'), $this->get_interpolation_js_array(), true, 'linear');
