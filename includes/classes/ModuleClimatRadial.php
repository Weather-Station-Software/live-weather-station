...
class Radial extends \WeatherStation\Engine\Module\Maintainer {
    ...
    public function __construct($station_information) {
        ...
        $this->module_hint = __('Display yearly temperature and precipitations as a beautiful radial graph.', 'live-weather-station');
    ...
    protected function get_datasource() {
        ...
        $content .= $this->get_assoc_option_select('climat-radial-datas-module-'. $this->station_guid, __('Module', 'live-weather-station'), $this->data, 0);
        $content .= $this->get_neutral_option_select('climat-radial-datas-measurement-'. $this->station_guid, __('Measurement', 'live-weather-station'));
        $content .= $this->get_key_value_option_select('climat-radial-datas-period-type-'. $this->station_guid, __('Period type', 'live-weather-station'), $this->get_period_type_js_array(false, true), true, 'fixed-month');
        $a_group = array();
        for ($i=1; $i<=$this->series_number; $i++) {
            $group = $this->get_neutral_option_select('climat-radial-datas-set-'. $i . '-'. $this->station_guid, __('Dataset', 'live-weather-station'));
            $group .= $this->get_neutral_option_select('climat-radial-datas-period-value-'. $i . '-' . $this->station_guid, __('Period', 'live-weather-station'));
            $group .= $this->get_key_value_option_select('climat-radial-datas-line-mode-' . $i . '-' . $this->station_guid, __('Mode', 'live-weather-station'), $this->get_line_mode_js_array(), true, 'line');
            $group .= $this->get_key_value_option_select('climat-radial-datas-dot-style-' . $i . '-' . $this->station_guid, __('Values display', 'live-weather-station'), $this->get_dot_style_js_array(), true, 'none');
            $group .= $this->get_key_value_option_select('climat-radial-datas-line-style-' . $i . '-' . $this->station_guid, __('Line style', 'live-weather-station'), $this->get_line_style_js_array(), true, 'solid');
            $group .= $this->get_key_value_option_select('climat-radial-datas-line-size-' . $i . '-' . $this->station_guid, __('Line size', 'live-weather-station'), $this->get_line_size_js_array(), true, 'regular');
            $a_group[] = array('content' => $group, 'name' => sprintf(__('Measurement %s', 'live-weather-station'), $i));
        }
        $content .= $this->get_group('climat-radial-datas-measure-group-', $a_group);
        $content .= '</tbody></table>';
    ...
    protected function get_parameters() {
        ...
        $content .= $this->get_key_value_option_select('climat-radial-datas-color-'. $this->station_guid, __('Color scheme', 'live-weather-station'), $this->get_colorbrewer_js_array(true));
        $content .= $this->get_key_value_option_select('climat-radial-datas-label-'. $this->station_guid, __('Label', 'live-weather-station'), $this->get_multi_label_js_array(), true, 'simple');
        $content .= $this->get_key_value_option_select('climat-radial-datas-guideline-'. $this->station_guid, __('Hint', 'live-weather-station'), $this->get_guideline_js_array(), true, 'standard');
        $content .= $this->get_key_value_option_select('climat-radial-datas-height-'. $this->station_guid, __('Height', 'live-weather-station'), $this->get_graph_size_js_array(), true, '300px');
        $content .= $this->get_key_value_option_select('climat-radial-datas-timescale-'. $this->station_guid, __('Time scale', 'live-weather-station'), $this->get_x_scale_js_array(false, false), true, 'auto');
        $content .= $this->get_key_value_option_select('climat-radial-datas-valuescale-'. $this->station_guid, __('Value scale', 'live-weather-station'), $this->get_y_scale_js_array(true), true, 'auto');
        $content .= $this->get_key_value_option_select('climat-radial-datas-interpolation-'. $this->station_guid, __('Interpolation', 'live-weather-station'), $this->get_interpolation_js_array(), true, 'none');
