...
class ColorBrewer {
    ...
    private static $colors = array('YlGn' => array(
    ...
    private static function _get($set, $size, $rank) {
        $color = '#000000';
        if (array_key_exists($set, self::$colors)) {
            if (array_key_exists($size, self::$colors[$set])) {
                if (array_key_exists($rank, self::$colors[$set][$size])) {
                    $color = self::$colors[$set][$size][$rank];
                }
            }
        }
        return $color;
