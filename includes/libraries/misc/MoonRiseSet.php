...
/**
 * Moon rise/set calculation class.
 * Adapted for PHP from JavaScript (http://bodmas.org/astronomy/riset.html  )
 *
...
class MoonRiseSet {
    ...
    public static function calculateMoonTimes($month, $day, $year, $lat, $lon) {
        ...
        //$timezone = (int)($lon / 15); --> compute it for UTC time
        $date = self::modifiedJulianDate($month, $day, $year);
        //$date -= $timezone / 24; --> compute it for UTC time
        $latRad = deg2rad($lat);
        $sinho = 0.0023271056;
        $sglat = sin($latRad);
        $cglat = cos($latRad);
